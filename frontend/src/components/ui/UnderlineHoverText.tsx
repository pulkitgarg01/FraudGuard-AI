...
import "./UnderlineHoverText.css";

export interface UnderlineHoverTextProps {
  text: string;
  /**
   * Resting text color. Tailwind / CSS class string or color.
   */
  textColor?: string;
  /**
   ...
   * class like `bg-zinc-900` (or any `bg-*`).
   */
  hoverColor?: string;
  fontSize?: string;
  fontWeight?: string;
...
const UnderlineHoverText: React.FC<UnderlineHoverTextProps> = ({
  ...
  return (
    <span
      className={cn(
        "group/underline underline-hover-wrapper relative inline-block cursor-pointer px-1 pb-1.5",
        "tracking-tight",
    ...
    >
      ...
      <span
        aria-hidden
        className="underline-hover-baseline pointer-events-none absolute bottom-0 left-0 h-px w-full bg-current opacity-25"
      />
      {/* Hover stroke — sweeps outward from the center */}
      <span
        aria-hidden
        className={cn(
          "underline-hover-stroke pointer-events-none absolute -bottom-px left-1/2 h-[3px] w-0 -translate-x-1/2 rounded-full",
